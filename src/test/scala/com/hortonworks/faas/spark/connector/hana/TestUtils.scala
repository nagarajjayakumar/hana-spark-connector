...
import com.hortonworks.faas.spark.connector.hana.sql.{HanaDbTable, TableIdentifier}
...
object TestUtils {
  ...
  def setupAllHanaDbTypes(test: TestBase, types: Seq[TestData.HanaDbType]): String = {
    val tbName = "alltypes"
    val tableDefn = HanaDbTable(TableIdentifier(tbName), types.map(_.columnDefn), Nil)
