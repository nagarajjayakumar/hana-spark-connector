...
/**
 * Representation for a key in HanaDb.
 */
sealed trait HanaDbKey {
 ...
 * `SHARD KEY`
 */
case class Shard(columns: Seq[ColumnReference]=Nil) extends HanaDbKey {
 ...
 * `KEY`
 */
case class Key(columns: Seq[ColumnReference]) extends HanaDbKey {
 ...
 */
case class KeyUsingClusteredColumnStore(columns: Seq[ColumnReference]) extends HanaDbKey {
 ...
 * `PRIMARY KEY`
 */
case class PrimaryKey(columns: Seq[ColumnReference]) extends HanaDbKey {
 ...
 * `UNIQUE KEY`
 */
case class UniqueKey(columns: Seq[ColumnReference]) extends HanaDbKey {
